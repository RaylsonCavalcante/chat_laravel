...
{   
    ...
    public function searchPatients(Request $req){
        ...
        if ($req->ajax()) {
            ...
            $patient = Patient::where('name', 'LIKE', '%'.$req->search.'%')->where('beautician_id', $req->id)->get();
            ...
            if(count($patient) > 0){
                ...
                foreach ($patient as $patients) {
                    ...
                    $facialHistoric = FacialHistoric::where('patient_id', $patients->id)->first();
                    $eyebrowHistoric = EyebrowHistoric::where('patient_id', $patients->id)->first();
                    $bodyHistoric = BodyHistoric::where('patient_id', $patients->id)->first();
                    
                    $output .= '<tr>
                                  <td>
                                    <div class="d-flex align-items-center">';

                                      if($patients->sex == "Masculino"){
                                        $output .= '<img src="/img/cli-m.png" alt="" style="width: 45px; height: 45px" class="rounded-circle"/>';
                                      }else{
                                        $output .= '<img src="/img/cli-f.png" class="rounded-circle" alt="" style="width: 45px; height: 45px" />';
                                      }

                                      $output .= '
                                      <div class="ms-3">
                                        <p class="fw-bold mb-1" id="nos">'.$patients->name.'</p>
                                        <p class="text-muted mb-0">'.$patients->email.'</p>
                                      </div>
                                    </div>
                                  </td>
                                  <td>
                                    ...
                                    <td>'.$patients->sex.'</td>
                                  <td>
                                    <a type="button" href="/editPatients/'.$patients->id.'" class="btn btn-link btn-rounded btn-sm fw-bold" data-mdb-ripple-color="dark">
                                      <i class="fas fa-edit"></i>
                                    </a>
                                    <button type="button" onclick="alertDeletePatient('.$patients->id.')" class="btn btn-link btn-rounded btn-sm fw-bold" data-mdb-ripple-color="dark">
                                      <i class="fas fa-trash text-danger"></i>
                                    </button>';

                                    if($facialHistoric or $eyebrowHistoric or $bodyHistoric){
                                    $output .='
                                    <a class="btn btn-link btn-rounded btn-sm fw-bold dropdown-toggle hidden-arrow align-items-center" href="#"
                                    id="pdfs" role="button" data-mdb-toggle="dropdown" aria-expanded="false">
                                    <i class="fas fa-print"></i>
                                      </a>
                                      <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="pdfs">
                                      ';

                                        if($facialHistoric){
                                        $output .= 
                                        '<li>
                                          <a class="dropdown-item" target="_blank" href="/dataFacialPdf/'.$patients->id.'">Dados Faciais</a>
                                        </li>';
                                        }
                                        ...
                                        $output .='
                                        <li>
                                          <a class="dropdown-item" target="_blank" href="/dataEyebrowPdf/'.$patients->id.'">Dados Sobrancelhas</a>
                                        </li>';
                                        }
                                        if($bodyHistoric){
                                        $output .='
                                        <li>
                                          <a class="dropdown-item" target="_blank" href="/dataBodyPdf/'.$patients->id.'">Dados Corporais</a>
                                        </li>';
                                        }
                                        $output .='
                                      </ul>';
                ...
                $output = ' <tr>
                              <td id="textSearch" scope="col" colspan="6">
                                  <div class="ms-3">
